...
class ExtensionHost {
    ...
    void destroy() {
        ...
        establishAndDestroyConnections(new HashSet<>());
        mAsyncLooper.quit();
    }

    private void establishAndDestroyConnections(Set<ComponentName> newExtensionNames) {
        // Get the list of active extensions
        Set<ComponentName> activeSet = new HashSet<>(newExtensionNames);

        // Get the list of connected extensions
        Set<ComponentName> connectedSet = new HashSet<>(mExtensionConnections.keySet());
    ...
    private Connection createConnection(final ComponentName cn, final boolean isReconnect) {
        ...
        conn.serviceConnection = new ServiceConnection() {
            ...
            public void onServiceConnected(final ComponentName componentName, IBinder iBinder) {
                ...
                execute(conn, extension -> {
                    ...
                    try {
                        ...
                        Log.e(TAG, "Error initializing extension " + componentName.toString(), e);
                    }
                }, 0, null);
                ...
                synchronized (conn.deferredOps) {
                    if (conn.ready) {
                        Set<Object> processedCollapsedTokens = new HashSet<>();
            ...
            public void onServiceDisconnected(final ComponentName componentName) {
                ...
                conn.binder = null;
                conn.ready = false;
                mClientThreadHandler.post(() -> mExtensionConnections.remove(componentName));
            }
        };

        try {
            if (!mContext.bindService(new Intent().setComponent(cn), conn.serviceConnection, Context.BIND_AUTO_CREATE)) {
    ...
    private void execute(final Connection conn, final Operation operation,
        ...
        if (collapseDelayMillis > 0 && collapseToken != null) {
            collapseTokenForConn = new Pair<>(conn.componentName,
                    collapseToken);
        } else {
        ...
        final Runnable runnable = () -> {
            try {
                ...
                Log.e(TAG, "Couldn't execute operation; scheduling for retry upon service reconnection.", e);
    ...
    void execute(ComponentName cn, Operation operation, int collapseDelayMillis, final Object collapseToken) {
        Connection conn = mExtensionConnections.get(cn);
        if (conn == null) {
    ...
    private static void _createUpdateOperation(final int reason) {
        UPDATE_OPERATIONS.put(reason, extension -> {
    ...
    static boolean supportsProtocolVersion(int protocolVersion) {
    ...
    private static class Connection {
         ...
         */
        final Queue<Pair<Object, Operation>> deferredOps = new LinkedList<>();
    }
}
